...
def step(step):
    ...
    if 0 < step < 7:
        template = f"onboarding/step{step}.html"
...
def get_libraries():
    ...
    current_app.logger.debug(
        f"Found {len(libraries)} libraries, enabled_libraries config: {config.enabled_libraries}"
    )
...
def save_hw_capabilities():
    ...
    try:
        ...
        if not capabilities_json:
            return jsonify(
                {"success": False, "message": "No capabilities data provided"}
            )
