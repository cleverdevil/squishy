...
def get_media_info(file_path: str) -> Dict[str, Any]:
    ...
    try:
        config = load_config()
        ffprobe_path = config.ffprobe_path or "ffprobe"  # Use config path or default to system ffprobe
        ...
        for stream in data.get("streams", []):
            ...
            if codec_type == "video":
                video_info = {
                    ...
                    "frame_rate": _parse_frame_rate(stream.get("avg_frame_rate", "0/1")),
...
def _extract_hdr_info(stream: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    for data in side_data:
        if data.get("side_data_type") == "DOVI configuration record" or "dv_profile" in data:
    ...
    if stream.get("codec_tag_string") == "dvh1" or stream.get("codec_name") == "dvhe" or "dvhe" in str(stream):
        hdr_info["type"] = "Dolby Vision"
        return hdr_info
...
def _detect_hdr_from_color_info(video_info: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if "dv_" in str(video_info) or "dolby" in str(video_info).lower() or "vision" in str(video_info).lower():
        hdr_info["type"] = "Dolby Vision"
        return hdr_info
    ...
    elif bit_depth and int(bit_depth) >= 10 and ("yuv420p10" in pixel_format or "p010" in pixel_format):
        hdr_info["type"] = "HDR (unspecified)"
